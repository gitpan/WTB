...
sub call
{
	my ($self, $function_name, @args) = _parse_wtb_args(@_);
...
sub output
{
	my ($self, $status, $info, $content, $mimetype) = _parse_wtb_args(@_);
...
sub get
{
	my ($self, $varname) = _parse_wtb_args(@_);
...
sub set
{
	my ($self, $varname, $value) = _parse_wtb_args(@_);
	return 1 if $self->{'sessiontable'} eq '';
...
sub unset
{
	my ($self, $name) = _parse_wtb_args(@_);
	return 1 if $self->{'sessiontable'} eq '';
...
sub fill
{
	my ($self, $template_name, $data) = _parse_wtb_args(@_);
...
sub _
{
	my ($self, $phrase, $language) = _parse_wtb_args(@_);
...
sub lang
{
	my ($self, $language) = _parse_wtb_args(@_);
	if (defined $language) {
		# set
		$self->set($language);
	}
...
sub translate
{
	my ($self, @pairs) = _parse_wtb_args(@_);
	return 0
		if $self->{'phrasetable'} eq '';
...
sub find
{
	my ($self, %options) = _parse_wtb_args(@_);
...
sub load
{
	my ($self, $group, $recordset, $tablename) = _parse_wtb_args(@_);
...
sub getparam
{
	my ($self, $name, $default, $regex) = _parse_wtb_args(@_);
...
sub login
{
	my ($self, $loginname, $password) = _parse_wtb_args(@_);
...
sub logmsg
{
	my ($self, $msg, $priority) = _parse_wtb_args(@_);
...
sub upload
{
	my ($self, $paramname, $groupname) = _parse_wtb_args(@_);
...
sub AUTOLOAD
{
	my ($self, @args) = _parse_wtb_args(@_);
	
	my $function_name = $WTB::AUTOLOAD;
	   $function_name =~ s/.*\://g;
	...
	if ($function_name eq 'DESTROY') {
		...
		foreach my $module (@{$self->{'modules'}}) {
			my $is_sub = 0;
			eval('$is_sub = (defined &WTB::Modules::'.ucfirst($module).'::'.$function_name.')');
			_die("eval failed: $@") if $@;
			if ($is_sub) {
				# call subroutine
				my @result;
				eval('@result = WTB::Modules::'.ucfirst($module).'::'.$function_name.'($self, @args)');
				_die("eval for subroutine call failed: $@") if $@;
				return (wantarray ? @result : (scalar @result ? $result[0] : undef));
			}
		}
		_die("could not find subroutine named 'WTB::Modules::*::".$function_name."'");
	}
...
# current WTB instance as first argument
sub _parse_wtb_args
{
	if (scalar @_ && ref($_[0]) eq 'WTB') {
		return @_;
	} else {
		return ($WTB, @_);
	}
...
# escapes a WTB field identifier, e.g. "mytable.myfield" or "myfield" etc.
sub _quotename
{
	my ($self, $fieldname) = @_;
	$self->_connect_to_db();
...
=head1 SYNOPSIS
	...
	use WTB;
	my $wtb = WTB->new( %options );
	print $wtb->handle();

=head1 DESCRIPTION

WTB tries to simplify the common tasks when creating dynamic
websites. The use of WTB should lead to the development of easy
to understand, relieable and fast dynamic web applications that
are easy to adjust and maintain.

WTB itself is designed to be as simple and straight
...
WTB was writted to provide abstractions and functionality for the
...
WTB relies on a common directory structure. This structure makes it
...
by WTB, usually this would go somewhere in the htdocs directory
on the server:
...
WTB, usually this would go B<outside> of the web-accessable area
on the server:
...
The methods of WTB can be invoced in two different styles:
...
When called as usual functions, the WTB instance the call works on
is a singleton and refers to the last instance of WTB that was
created via the new() constructor.

The functional way of invocation has the advantage of beeing
visually more obvious. Usually there is only one instance of WTB
...
In WTB, each time a webpage is requested, the workflow engine tries
...
value that tells WTB to forward control flow directly to another
workflow function.

Inside a workflow function the WTB instance is magically
available as the variable $wtb and the additional arguments
are stored in @args.
...
Examples for workflow functions:
	...
	# ...
	return $wtb->output(1, 'ok', "<h1>Hello, World!</h1>");

	# ...
	return $wtb->followup("core.error");

	# ...
	return $wtb->output(1, 'ok', "...", 'image/png');
...
return its result unmodified and unanalysed.
	...
	my $result = $wtb->call( $name, @args );


=head3 output()

This method returns a valid WTB result that can be returned from
...
It gets the following options:
	...
	# ...
	return $wtb->output(1, 'info...', $html, 'text/html');
...
It gets the following options:
	...
	# ...
	return $wtb->followup('group.function', @args);
...
a unhandy, for example:
	...
	WTB::Modules::MyProjectModule::my_tiny_function($wtb, @args);

Plus, in  most cases, you have to pass the WTB instance to the
...
WTB will try one module after another until it has found the
...
throughout several webpage requests. In the WTB the session is a flat hash
...
is usually I<sid>, but can be configured. WTB takes automaticly care of
...
When the WTB instance is created, the session is loaded automaticly and
can be accessed through the following methods:

=head3 get()

	my $value = $wtb->get( "my_name" );

=head3 set()

	$wtb->set( "my_name", "value" );
...
are replaced by actual values when the template is I<filled>. In WTB
...
some examples for fill() calls:
	...
	my $string1 = $wtb->fill( $name1,   $hash );
	my $string2 = $wtb->fill( $name2, [ $hash, $hash ] );
...
In WTB Themes are sets of templates. Each theme has its own subdirectory
...
statements mean the same:
	...
	my $string = $wtb->fill('form.date', {-month => '...', -year => '...'});
	
	my $string = $wtb->FormDate(-month => '...', -year => '...');
...
To access the data that is stored inside the attached (relational) database, WTB
offers some handy functions.
When the WTB instance is created, all information regarding the database
connection is given and WTB will try to establish a connection.
...
When inserted in the database, WTB checks first, if a certain row
...
The appropriate access function is then executed and tells WTB
...
the special variable $wtb is magically available, which is the
WTB instance. The subroutine body should return either 1 or 0.
...
Use the search on cpan.org to find alternatives to the WTB.
